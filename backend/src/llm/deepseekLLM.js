// DeepSeek LLM — deepseek-chat (compatible OpenAI, fallback : mockLLM)
...
function init() {
  ...
  if (!apiKey) {
    console.log('⚠️  DEEPSEEK_API_KEY absent — décisions rule-based (mock)');
    return false;
  }
  console.log(`✓ DeepSeek initialisé (${MODEL})`);
  return true;
}
...
async function decide(context) {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`DeepSeek ${res.status} → mock`);
      if (res.status === 429) console.warn('DeepSeek quota dépassé — mock activé');
    ...
    console.warn(`DeepSeek → mock (${err.message})`);
    return mockDecide(context);
  }
}
