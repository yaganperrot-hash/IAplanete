// Groq LLM — Llama 3.1 8B Instant (gratuit : 14 400 req/jour)
...
function init() {
  ...
  if (!apiKey) {
    console.log('⚠️  GROQ_API_KEY absent — décisions rule-based (mock)');
    return false;
  }
  console.log(`✓ Groq initialisé (${MODEL})`);
  return true;
}
...
function buildPrompt(ctx) {
  ...
  // Évaluation des niveaux de besoin
  const securite = ctx.nearbyThreats > 0
    ? (ctx.health < 25 ? 'DANGER_IMMÉDIAT' : 'menace_détectée')
    : 'sûr';
  ...
  // Actions disponibles par niveau de priorité
...
async function decide(context) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.warn(`Groq ${res.status} → mock`);
      if (res.status === 429) console.warn('Groq quota dépassé — mock activé');
    ...
    console.warn(`Groq → mock (${err.message})`);
    return mockDecide(context);
  }
}
